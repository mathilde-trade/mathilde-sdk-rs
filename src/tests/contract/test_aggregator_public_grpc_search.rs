...
struct CapturedSearchRequest {
    ...
    body: proto::SearchBarsRequestV1,
}

#[derive(Debug, Clone)]
enum SearchGrpcUnaryReply {
    Success(proto::BarsSearchResponseV1),
    Status {
        code: tonic::Code,
        message: &'static str,
    },
...
async fn spawn_search_grpc_server(
    reply: SearchGrpcUnaryReply,
) -> (String, oneshot::Receiver<CapturedSearchRequest>) {
        ...
        let service = service_fn(move |request: Request<Incoming>| {
            ...
            async move {
                ...
                let body_bytes = request
                    ...
                    .to_bytes();
                let decoded = decode_grpc_message::<proto::SearchBarsRequestV1>(&body_bytes);
                ...
                let response = match reply {
                    SearchGrpcUnaryReply::Success(message) => Response::builder()
                        .status(200)
                        .header("content-type", "application/grpc")
                        .header("grpc-status", "0")
                        .body(Full::new(Bytes::from(encode_grpc_message(message))))
                        .expect("grpc success response"),
                    SearchGrpcUnaryReply::Status { code, message } => Response::builder()
                        .status(200)
                        .header("content-type", "application/grpc")
...
async fn test_search_bars_grpc_uses_unary_path_and_decodes_min_response() {
    ...
    assert_eq!(
        captured.path,
        "/mathilde.feed.bars.v1.BarsServiceV1/SearchBars"
    );
    assert_eq!(
...
#[tokio::test]
async fn test_search_bars_grpc_maps_non_ok_grpc_status() {
    let (base_url, _) = spawn_search_grpc_server(SearchGrpcUnaryReply::Status {
        code: tonic::Code::InvalidArgument,
        message: "predicate must be non-empty",
    })
    .await;

...
use crate::systems::regime::{Regime, RegimeOutput, TimeMachineOutputsGrpcRequest};
...
#[tokio::test]
async fn test_regime_time_machine_outputs_grpc_uses_unary_path_and_decodes_rows() {
    ...
    match &out.rows[0].output {
        RegimeOutput::WithMeta(output) => assert_eq!(output.metadata.source, "feed"),

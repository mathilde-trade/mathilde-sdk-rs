...
use crate::systems::regime::{
    MessagesWsServerFrame, MessagesWsSubscribeFrame, MessagesWsUnsubscribeFrame, Regime,
};
...
#[tokio::test]
async fn test_regime_connect_messages_ws_sends_auth_and_control_frames_and_decodes_server_frames() {
    ...
    match subscribed {
        MessagesWsServerFrame::Subscribed(frame) => {
            ...
            assert_eq!(frame.tfs, vec!["1h".to_string()]);
        }
...
#[tokio::test]
async fn test_regime_recovering_messages_ws_replays_active_subscriptions_after_disconnect() {
    let (base_url, captured_rx) = spawn_recovering_messages_ws_server().await;
    let client = Regime::new(config_for_ws(&base_url, None)).expect("regime client");
    ...
    let _ = connection
        ...
        .expect("server frame");
    let captured = captured_rx.await.expect("captured subscriptions");

    assert_eq!(captured.len(), 2);
    let first: serde_json::Value = serde_json::from_str(&captured[0]).expect("first subscribe");
    let second: serde_json::Value = serde_json::from_str(&captured[1]).expect("replayed subscribe");
    assert_eq!(first, second);
    assert_eq!(first["tfs"], serde_json::json!(["1h"]));
}
